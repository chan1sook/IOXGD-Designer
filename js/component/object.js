addComponent({
      ...
      $(element).css({
        ...
        background:
          +this.property.grad_dir === 0
            ? hexToRgbA(this.property.main_color, this.property.opacity / 255)
            : `linear-gradient(${
                +this.property.grad_dir === 1 ? "90" : "180"
              }deg, ${hexToRgbA(
                this.property.main_color,
                this.property.opacity / 255
              )} 0%, ${hexToRgbA(
                this.property.grad_color,
                this.property.opacity / 255
              )} 100%)`,
        // opacity: this.property.opacity / 255,
        border: `${this.property.border_width}px solid ${hexToRgbA(
          this.property.border_color,
          this.property.border_opacity / 255
        )}`,
        "border-radius": `${this.property.radius}px`,
        "box-shadow": `0 ${
          this.property.shadow_type == 0 ? this.property.shadow_width : 0
        }px ${this.property.shadow_width}px ${this.property.shadow_color}`,
        overflow: "hidden",
        position: "absolute",
      });
  ...
  build: async function (simulator, pagename, output_path) {
    ...
    code += `${this.property.name} = lv_obj_create(${
    ...
    code += `lv_obj_set_style_local_bg_color(${
      this.property.name
    }, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x${this.property.main_color.substring(
      1
    )}));\n`;
    code += `lv_obj_set_style_local_bg_grad_color(${
      this.property.name
    }, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x${this.property.grad_color.substring(
      1
    )}));\n`;
    ...
    code += `lv_obj_set_style_local_border_color(${
      this.property.name
    }, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x${this.property.border_color.substring(
      1
    )}));\n`;
    ...
    code += `lv_obj_set_style_local_shadow_color(${
      this.property.name
    }, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x${this.property.shadow_color.substring(
      1
    )}));\n`;
